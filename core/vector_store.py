...
# ─── CONFIG ──────────────────────────────────────────────────
EMBED_MODEL = "all-MiniLM-L6-v2"   # fast, good quality
...
def search(query: str, top_k: int = 5,
    ...
    # HyDE: use hypothetical answer for embedding
    ...
    if filter_doc:
        ...
        candidates = min(top_k * 3, index.ntotal)
    distances, indices = index.search(q_vec, candidates)
...
def generate_hypothetical_answer(query: str) -> str:
    ...
    The hypothetical answer embeds closer to real doc chunks.
    """
    try:
        response = _hyde_client.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=200,
            messages=[{
                "role": "user",
                "content": f"""Write a short hypothetical answer to this question 
as if it came from a pharmaceutical regulatory document.
2-3 sentences only. Be specific and technical.

Question: {query}"""
            }]
        )
        result = response.content[0].text
        print(f"  HyDE generated: {result[:100]}")
